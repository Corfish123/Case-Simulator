...
const ChanceCardsRarityWeaponName = document.querySelectorAll(
  ".SkinItem-Chance-Weapon"
);
const ChanceCardsRaritySkinName = document.querySelectorAll(
...
for (let i in CaseData) {
  ...
  CaseGrid_Case_Cloned.querySelector(".CasesGrid_Case_IMG img").src =
    CaseData[i].image_url;
  CaseGrid_Case_Cloned.querySelector(
    ".CasesGrid_Case_Rarity_Name"
  ).textContent = CaseData[i].name;
}
...
function Open_Case_Function(CaseNumber) {
  ...
  function Random_Knife_Function() {
    RandomKnife = Math.floor(
      Math.random() * Current_Case_Data["Rare Special Items"].length
    );
  ...
  function Load_Case_Skins() {
    ...
    for (let i = 0; i < 1; i++) {
      function Styling_Skin_Card_Function(
        Skin_Rarity_Name,
        index,
        background_Color
      ) {
        ...
        ChanceCardsPrizeIMG[TotalSkins - 1].src = Object.values(
          Current_Case_Data[Skin_Rarity_Name][index].wears
        )[0];
        let NameArray =
          Current_Case_Data[Skin_Rarity_Name][index].name.split("|");
        ...
        ChanceCardsRarityDOM[TotalSkins - 1].style.backgroundColor =
          background_Color;
      }
      ...
      for (let i = 0; i < 1; i++) {
        ...
        ChanceCardsPrizeIMG[TotalSkins - 1].src =
          "PNGS/Yellow_Special_Item.png";
        ChanceCardsRarityDOM[
          TotalSkins - 1
        ].style.backgroundColor = `rgb(199,160,9)`;
        ChanceCardsRarityWeaponName[TotalSkins - 1].textContent =
          "★Rare Special Item★";
  ...
  function WorkingCase() {
    ...
    function RandomRarityFunction() {
      ...
      if (
        RandomRarityNumber > BlueOdds * OddsMultiplyingValue &&
        RandomRarityNumber <= MaxOdds
      )
        return (CurrentRarity = RaritiesArray[0]);
      if (
        RandomRarityNumber > PurpleOdds * OddsMultiplyingValue &&
        RandomRarityNumber <= BlueOdds * OddsMultiplyingValue
      )
        return (CurrentRarity = RaritiesArray[1]);
      if (
        RandomRarityNumber > PinkOdds * OddsMultiplyingValue &&
        RandomRarityNumber <= PurpleOdds * OddsMultiplyingValue
      )
        return (CurrentRarity = RaritiesArray[2]);
      if (
        RandomRarityNumber > RedOdds * OddsMultiplyingValue &&
        RandomRarityNumber <= PinkOdds * OddsMultiplyingValue
      )
        return (CurrentRarity = RaritiesArray[3]);
      if (
        RandomRarityNumber > MinOdds &&
        RandomRarityNumber <= RedOdds * OddsMultiplyingValue
      )
        return (CurrentRarity = RaritiesArray[4]);
    }
    ...
    function setPrizeStyles(index, PrizeImage, PrizeRarity) {
      ...
      Random_MilSpec_Item = Math.floor(
        Math.random() * Current_Case_Data["Mil-Spec Skins"].length
      );
      Random_Restricted_Item = Math.floor(
        Math.random() * Current_Case_Data["Restricted Skins"].length
      );
      Random_Classified_Item = Math.floor(
        Math.random() * Current_Case_Data["Classified Skins"].length
      );
      Random_Covert_Item = Math.floor(
        Math.random() * Current_Case_Data["Covert Skins"].length
      );

      //CARD NAME DOMS
      const PrizeRarity_Weapon = document.querySelectorAll(
        ".PrizeRarity-Weapon"
      );
      const PrizeRarity_SkinName = document.querySelectorAll(
      ...
      function Styling_Card(Skin_Rarity, background_Color, Random_Item_Index) {
        let NameArray =
          Current_Case_Data[Skin_Rarity][Random_Item_Index].name.split("|");
        PrizeRarity.style.backgroundColor = background_Color;
        PrizeImage.src = Object.values(
          Current_Case_Data[Skin_Rarity][Random_Item_Index].wears
        )[0];
        ...
        if (PrizeRarity_Weapon[index].parentNode.parentNode === Winner_Card) {
          PrizeRarity_Weapon[index].parentNode.parentNode.Item_Description =
            Current_Case_Data[Skin_Rarity][Random_Item_Index].desc;
        }
      }
      ...
      for (let i = 0; i < 1; i++) {
        if (CurrentRarity == RaritiesArray[0])
          Styling_Card(
            Current_Case_Data_Rarities[0],
            `rgb(61,87,195)`,
            Random_MilSpec_Item
          );
        if (CurrentRarity == RaritiesArray[1])
          Styling_Card(
            Current_Case_Data_Rarities[1],
            `rgb(107,72,195)`,
            Random_Restricted_Item
          );
        if (CurrentRarity == RaritiesArray[2])
          Styling_Card(
            Current_Case_Data_Rarities[2],
            `rgb(166, 60, 174)`,
            Random_Classified_Item
          );
        if (CurrentRarity == RaritiesArray[3])
          Styling_Card(
            Current_Case_Data_Rarities[3],
            `rgb(181,73,64)`,
            Random_Covert_Item
          );
    ...
    function CardRollAnimation() {
      ...
      for (let i = 0; i < Cards.length; i++) {
        ...
        if (Cards[i] == Winner_Card) {
          ...
          console.log(
            "Cards Rarity is :" + CurrentRarity + " " + RandomRarityNumber
          );
          ...
          if (CurrentRarity != "Rare Special Item") {
            ...
            ItemName =
              Winner_Card_Name_Array[1].replace(/\s+/, " ").trim() +
              " | " +
              Winner_Card_Name_Array[2].replace(/\s+/, " ").trim();
            PrizeContainerName.textContent = ItemName;

            W_RGB = Winner_Card_Name.style.backgroundColor.split(/[\s,()]+/);
            PrizeContainer.style.background = `linear-gradient(0deg, rgba(38,38,38,.9) 0%, rgba(38,40,43,1) 7%, rgba(32,32,32,1) 18%, rgba(${
              W_RGB[1] - 23
            },${W_RGB[2] - 46},${W_RGB[3] - 94}) 80%, rgba(${W_RGB[1] - 23},${
              W_RGB[2] - 46
            },${W_RGB[3] - 94}) 97%, rgba(${W_RGB[1] - 23},${W_RGB[2] - 46},${
              W_RGB[3] - 94
            },0.95) 100%) ,
            ...
            if (
              Current_Case_Data["Rare Special Items"][RandomKnife]
                .can_be_stattrak === false
            ) {
              Winner_Card.StatTrack = "false";
              console.log("GAY");
            }
            PrizeContainerIMG.src = Object.values(
              Current_Case_Data["Rare Special Items"][RandomKnife].wears
            )[0];
            Winner_Card_Name_Array = Current_Case_Data["Rare Special Items"][
              RandomKnife
            ].name.replace("★ ", "");
            ...
            Winner_Card.Item_Description =
              Current_Case_Data["Rare Special Items"][RandomKnife].desc;
            PrizeContainerName.textContent = ItemName;
      ...
      CaseCards.style.transform = `translate(-${
        CardWidth * 24.5 + (CardWidth / 14) * 23 + RandomPixel
      }px)`;
    }
  ...
  ContinueButton.onclick = () => {
    ...
    setTimeout(() => {
      ChanceSkins.forEach(addAnimation);
    }, 100);
  ...
  function OpenButtonFunction() {
    ...
    const OpeningCaseContainer = document.querySelector(
      ".OpeningCaseContainer"
    );
    observer.observe(OpeningCaseContainer, {
      attributes: true,
